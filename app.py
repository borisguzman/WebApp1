...
import matplotlib
matplotlib.use("Agg") # works better with streamlit
...
def summarize_text(text, num_sentences=3):
    ...
    # Calculate the frecuency os each word
    word_freq  = Counter(words)

    # Sort the words based on their frequency in descending order
    sorted_words = sorted(word_freq, key=word_freq.get, reverse=True)

    # Extract the top num_sentences most frequent
...
def text_analyzer(text):
    ...
    allData = [('"Token":{},\n"Lemma":{}'.format(token.text, token.lemma)) for token in doc]
    return allData
...
def main():
    ...
    <h1 style="color:blue">NLP Web App</h1>
    </div>
    """
    st.markdown(subheader_template, unsafe_allow_html=True)

    st.sidebar.image("nlp.png", use_container_width=True)
    ...
    if choice == "Text Analysis":
        ...
        if st.button("Analyze", "BtnAnalize"):
            if len(raw_text) == 0:
                ...
                with col1:
                    ...
                    with st.expander("Stopwords"):
                        ...
                        stop_w = nt.extract_stopwords(raw_text)
                        st.error(stop_w)

                with col2:
                    with st.expander("Processed Text"):
                        st.success("Stopwords Exluded Text")
    ...
    if choice == "Translation":
        ...
        if len(raw_text) < 3:
            st.warning("Please provide a text with as least 3 characters...")
        else:
            ...
            if st.button("Translade"):
                translator = GoogleTranslator(source='auto', target=target_lang)
                transalted_text = translator.translate(raw_text)
                st.write(transalted_text)
